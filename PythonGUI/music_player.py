...
play_layout = [[sg.VPush()],
               [sg.Push(), sg.Text(song_name, font = 'Young'), sg.Push()],
               [sg.VPush()],
               [sg.Push(),
                sg.Button(image_data = base64_image_import('./extras/play.png'), key = '-PLAY-', button_color = 'white', border_width = 0), 
                sg.Text(),
                sg.Button( image_data = base64_image_import('./extras/pause.png'), key = '-PAUSE-', button_color = 'white', border_width = 0),
                sg.Push()
                ],[sg.VPush()],
               [sg.Progress(song_length, size = (20,20), key = '-PROGRESS-')]
            ]

volume_layout = [[sg.VPush()],
                 [sg.Push(),sg.Slider(range = (0, 100), default_value = 100, orientation = 'h', key = '-VOLUME-'), sg.Push()],
                 [sg.VPush()]]

layout = [[sg.TabGroup([[sg.Tab('Play', play_layout), sg.Tab('Volume', volume_layout)]])],
          ]

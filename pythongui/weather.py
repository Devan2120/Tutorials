...
info_col = sg.Column([
    ...
    [sg.Text('', key = '-TIME-', font = 'Calibri 16', text_color = '#FFFFFF', background_color = '#000000', pad = 0, visible = False)],
    [sg.Text('', key = '-TEMP-', font = 'Calibri 16', text_color = '#000000', background_color = '#FFFFFF', pad = (0,10), justification = 'center', visible =False)]
    ]) 
layout = [[sg.Input(expand_x = True, key = '-INPUT-'), sg.Button('Enter', button_color = '#000000', border_width = 0)],
          [image_col, info_col]]
...
while True:
    ...
    if event == 'Enter':
        ...
        if weather in ('Sun', 'Sunny', 'Clear', 'Clear with Periodic clouds'):
            window['-IMAGE-'].update("./extras/sun.png", visible = True)

        if weather in ('Mostly Sunny', 'Partly Sunny', 'Partly Cloudy', 'Cloudy', 'Mostly Cloudy', 'Overcast'):
            window['-IMAGE-'].update("./extras/part_sun.png", visible = True)

        if weather in ('Rain', 'Chance of Rain', 'Light Rain', 'Showers', 'Scattered Showers', 'Rain and Snow', 'Hail'):
            window['-IMAGE-'].update("./extras/rain.png", visible = True)

        if weather in ('Sacttered Thunderstorms', 'Chance of Storm', 'Storm', 'Thunderstorm', 'Chance of TStorm'):
            window['-IMAGE-'].update("./extras/thunder.png", visible = True)

        if weather in ('Mist', 'Dust', 'Fog', 'Smoke', 'Haze', 'Flurries'):
            window['-IMAGE-'].update("./extras/fog.png", visible = True)

        if weather in ('Freezing Drizzle', 'Chance of Snow', 'Sleet', 'Snow', 'Icy', 'Snow Showers'):
            window['-IMAGE-'].update("./extras/snow.png", visible = True)
